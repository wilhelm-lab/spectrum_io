...
def main(args):
    ...
    if args.prosit_input:
        ...
        writer_with_proteins.writerow("modified_sequence,collision_energy,precursor_charge,protein,fragmentation".split(","))
        
        pre, not_post = get_cleavage_sites(args.enzyme)    
    ...
    # python digest.py --fasta /media/kusterlab/internal_projects/active/Mouse_proteome/stuff/10090_UP000000589_UniProtKB_Mouse_CanIso_2018_03_27.fasta 
    ...
    if args.peptide_protein_map:
        ...
        pre, not_post = get_cleavage_sites(args.enzyme)    
        for peptide, proteins in get_peptide_to_protein_map(args.fasta, db = 'concat', digestion = args.digestion,
         min_len = args.min_length, max_len = args.max_length, pre = pre, not_post = not_post, miscleavages = args.cleavages, 
         methionine_cleavage = True, special_aas = list(args.special_aas), use_hash_key = False).items():
...
def parse_args(call):
    ...
    apars.add_argument('--db', metavar = "M", required = False, default="concat", 
                                         help='''Target/decoy/concat''')                                                                                            
    add_arguments(apars)                                                                                                
...
def parse_uniprot_id(fastaId):
    """Parse uniprot id."""
    protein_id = parse_until_first_space(fastaId)
    if "|" in protein_id:
        return protein_id.split("|")[1]
    else:
        return protein_id

def read_fasta_proteins(file_path, db = "concat", parse_id = parse_until_first_space, parseprotein_name = parseprotein_name_func, 
...
def read_fasta_maxquant(file_path, db: str = "target", parse_id = parse_until_first_space, special_aas = ['K', 'R'], 
...
def get_peptides(fasta_file, db = "concat", min_len = 6, max_len = 50, pre = ['K', 'R'], not_post = ['P'], digestion = 'full', 
miscleavages = 0, methionine_cleavage = True):
    """Get peptides."""
    for _, seq in read_fasta(fasta_file, db):
        for peptide in get_digested_peptides(seq, min_len, max_len, pre, not_post, digestion, miscleavages, methionine_cleavage):
            yield peptide


def get_digested_peptides(seq, min_len = 6, max_len = 50, pre = ['K', 'R'], not_post = ['P'], digestion = 'full', 
miscleavages = 0, methionine_cleavage = True):
...
def get_peptide_to_protein_map(fasta_file, db = "concat", min_len = 6, max_len = 52, pre = ['K', 'R'], not_post = ['P'], 
digestion = 'full', miscleavages = 2, methionine_cleavage = True, use_hash_key = False, special_aas = ['K', 'R'], 
...
def get_ibaq_peptide_to_protein_map(args):
    ...
    pre, not_post = get_cleavage_sites(args.enzyme)
    return get_peptide_to_protein_map(args.fasta, db = 'concat', digestion = 'full', min_len = max([6, args.min_length]), 
    max_len = min([30, args.max_length]), pre = pre, not_post = not_post, miscleavages = 0, methionine_cleavage = False, 
    special_aas = list(args.special_aas))


def get_num_ibaq_peptides_per_protein(args):
    """Get number of ibaq peptides per protein."""
    peptide_to_protein_mapIbaq = get_ibaq_peptide_to_protein_map(args)
    return get_num_peptides_per_protein(peptide_to_protein_mapIbaq)
...
def is_enzymatic(aa1, aa2, pre = ['K', 'R'], not_post = ['P'], methionine_cleavage: bool = True) -> bool:
    """Check if is enzymatic."""
    return aa1 == "-" or aa2 == "-" or (aa1 in pre and aa2 not in not_post) or (methionine_cleavage and aa1 == "M")


def has_miscleavage(seq, pre = ['K', 'R'], not_post = ['P']):
